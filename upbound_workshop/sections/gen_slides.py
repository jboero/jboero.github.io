...
OUT = "/home/jboero/code/jboero.github.io/upbound/sections"
...
def updot(x, y, s=1.0):
    """Small 'up' mark: the dot + 'up' text, echoing the logo."""
    return (f'  <circle cx="{x}" cy="{y-26*s}" r="{7*s}" fill="{WHITE}"/>\n'
            f'  <text x="{x-9*s}" y="{y}" {FONT} font-size="{34*s}" '
            f'font-weight="700" fill="{WHITE}">up</text>\n')
...
slides["1/1"] = (head() + panel(0.82)
    # big wordmark - tspans keep natural kerning; dot rides above the 'u'
    + f'  <circle cx="290" cy="185" r="15" fill="{WHITE}"/>\n'
    + f'  <text x="480" y="288" text-anchor="middle" {FONT} font-size="110" font-weight="700">'
      f'<tspan fill="{WHITE}">up</tspan><tspan fill="{TITLE}">bound</tspan></text>\n'
    + f'  <rect x="300" y="318" width="360" height="5" rx="2.5" fill="{BRAND}"/>\n'
    + f'  <text x="480" y="372" text-anchor="middle" {FONT} font-size="30" font-weight="700" fill="{BODY}">The Universal Cloud Platform</text>\n'
    + f'  <text x="480" y="410" text-anchor="middle" {FONT} font-size="21" fill="{DIM}">Control planes for platform engineering</text>\n'
    + f'  <text x="480" y="458" text-anchor="middle" {FONT} font-size="19" fill="{DIM}">John Boero &#8226; 2026</text>\n'
    + "</svg>\n")
...
def arrow(x1, y1, x2, y2):
    return (f'  <line x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" stroke="{BORDER}" '
            f'stroke-width="3"/>\n'
            f'  <path d="M {x2} {y2} l -12 -7 l 0 14 z" fill="{BORDER}" '
...
slides["3/3"] = (head() + panel(0.82)
    + f'  <circle cx="370" cy="122" r="9" fill="{WHITE}"/>\n'
    + f'  <text x="480" y="182" text-anchor="middle" {FONT} font-size="64" font-weight="700">'
      f'<tspan fill="{WHITE}">up</tspan><tspan fill="{TITLE}">bound</tspan></text>\n'
    + f'  <text x="480" y="248" text-anchor="middle" {FONT} font-size="34" font-weight="700" fill="{BODY}">Own your cloud platform.</text>\n'
...
slides["4/1"] = (head() + panel()
    + f'  <circle cx="480" cy="176" r="12" fill="{WHITE}"/>\n'
